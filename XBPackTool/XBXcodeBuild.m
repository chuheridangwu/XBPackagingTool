...
+ (void)xcodeBuildProjectSetBuildType:(NSString*)buildType{
    ...
    // 1.清理工程
        NSString *clean = [NSString stringWithFormat:@"/usr/bin/xcodebuild -workspace ManMao.xcworkspace -scheme %@  clean",XB_ProjectName];
    
    // 2.编译项目
        NSString *build = [NSString stringWithFormat:@"/usr/bin/xcodebuild -workspace ManMao.xcworkspace -scheme %@ -sdk iphoneos -configuration %@ build" ,XB_ProjectName,buildType];
